...
const WordPage = () => {
  ...
  const fetchDocuments = async () => {
    setLoading(true);
    try {
      const response = await api.word.getDocuments();
      console.log('Word documents response:', response);
  ...
  const handleDeleteDocument = async (e, documentId) => {
    ...
    try {
      await api.word.deleteDocument(documentId);
  ...
  return (
    <div className="container mx-auto">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">Tài liệu Word</h1>
        <p className="text-gray-600">Quản lý và xử lý tài liệu Word của bạn</p>
      </div>
        ...
        <div>
          <DocumentProcessingTabs
            ...
            api={{
              ...
              addWatermark: api.word.addWatermark
            }}
          />
        </div>
      </div>
